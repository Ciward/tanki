...
class Bullet(pygame.sprite.Sprite):
    ...
    def move(self):
        ...
        if self.rect.left <= -game.WINDOWHEIGHT:
            self.kill()
        elif self.rect.right >= game.WINDOWWIDTH*2:
            self.kill()
        if self.rect.top <= -game.WINDOWHEIGHT:
            self.kill()
        elif self.rect.bottom >= game.WINDOWHEIGHT*2:
            self.kill()
...
class Missle(Bullet):
    ...
    def dirt(self):
        ...
        mydirt.life = 20
        if self.live%20==0:self.radangle1=(self.radangle1+180)%360
...
class Gauss(Gun):
    ...
    def realshoot(self):
        ...
        pow = self.owner.bombpower * (0.5 +  self.p)
...
class Electric(Dirt):
    def __init__(self, owner, pos, len, angle):
        ...
        if len <= 300:
            ...
            self.life = 20
        elif len>=1200:self.life=0
        self.index = 1
        if self.life>0:
            for img in self.originalimages:
                # original = img
                self.images.append(pygame.transform.rotate(pygame.transform.smoothscale(img, (int(len), 18)), -self.angle))
...
class Firegun(Gun):
    ...
    def shoot(self):
        ...
        for i in self.anglelist:
            if self.owner.iscrazy:
                Fire(self, x, y, i, self.firelive,
                     self.rand(self.owner.firepower * 4, int(self.owner.firepower * 2 * 0.5)), self.speed)
            else:
                Fire(self, x, y, i, self.firelive, self.rand(self.owner.firepower * 2, int(self.owner.firepower * 0.5)),
                     self.speed)
        self.starcenter = (self.rect.centerx + self.star_r * math.cos(self.radangle),
                           self.rect.centery + self.star_r * math.sin(self.radangle))
...
class Magnum(Gun):
    ...
    def shoot(self):
        ...
        x = self.rect.centerx + self.r * math.cos(self.radangle)
        y = self.rect.centery + self.r * math.sin(self.radangle)
        Bomb(self, x, y, self.angle, 500, self.rand(self.owner.bombpower, int(self.owner.bombpower * 0.5)),
             self.blspeed)
        self.time = 0
...
class Meltgun(Gun):
    ...
    def shoot(self):
        ...
        y = self.rect.centery + self.r * math.sin(self.radangle0)
        if self.owner.isen:
            enShortlight(self, x, y, self.angle, self.bllive,
                         self.rand(enShortlight.enpower, int(enShortlight.enpower * 0.5)), self.blspeed)
        else:
            Shortlight(self, x, y, self.angle, self.bllive,
                       self.rand(self.owner.shortltpor*2, int(self.owner.shortltpor * 0.5)), self.blspeed)
        self.time = 0
...
class Railgun(Gun):
    ...
    def showstar(self):
        self.star_center = (self.rect.centerx + self.star_r * math.cos(self.angle * math.pi / 180),
                            self.rect.centery + self.star_r * math.sin(self.angle * math.pi / 180))
    ...
    def realshoot(self):
        ...
        if type(self.owner) == Enemytank1:
            ...
            power=self.rand(int(self.owner.ltpor*1.5),int(self.owner.ltpor*0.4))
            Light(self, x, y, self.angle, 500,power, self.speed)
        self.ispre = False
...
class Star(pygame.sprite.Sprite):
    def __init__(self, owner):
        ...
        self.star_r = 45
        self.image = self.images[1]
        self.center = (self.owner.rect.centerx + self.star_r * math.cos(self.owner.angle * math.pi / 180),
                       self.owner.rect.centery + self.star_r * math.sin(self.owner.angle * math.pi / 180))
        self.rect = self.image.get_rect(center=self.center)
...
class Enemy(Tank):
    ...
    def __init__(self):
        ...
        self.isen = True
        self.rect.x = random.randint(0, game.WINDOWWIDTH)
        self.rect.y = random.randint(0, game.WINDOWHEIGHT)
        self.lightmark = None
        self.expmark = None
        self.still = 0
        self.shootime = 0
...
class Enemy0(Enemy):
    ...
    def move(self):
        ...
        if self.rect.x < -game.WINDOWWIDTH*2:
            self.kill()
        elif self.rect.x + 60 > game.WINDOWWIDTH*3:
            self.kill()
        if self.rect.y < -game.WINDOWHEIGHT*2:
            self.kill()
        elif self.rect.y + 60 > game.WINDOWHEIGHT*3:
            self.kill()
...
class Enemytank0(Enemy):
    ...
    def move(self):
        ...
        if random.choice([True, False]):
            self.moveU()
        self.turn()
        if self.rect.centerx < -game.WINDOWWIDTH*2:
            self.kill()
        elif self.rect.centerx > game.WINDOWWIDTH*3:
            self.kill()
        if self.rect.centery < -game.WINDOWHEIGHT*2:
            self.kill()
        elif self.rect.centery > game.WINDOWHEIGHT*3:
            self.kill()
...
class Boss(Enemytank0):
    ...
    def __init__(self):
        ...
        self.call()
        self.cure()
        self.guns = [Meltgun(self, player.rect.center), Meltgun(self, player.rect.center),
                     Meltgun(self, player.rect.center), Meltgun(self, player.rect.center)]
    ...
    def showlive(self):
        if game.live > 0:
            ...
            self.textrect = self.text.get_rect(center=(self.rect.centerx, self.rect.centery - 100))
...
class Player(Tank):
    def __init__(self):
        ...
        self.rect.center=(game.WINDOWWIDTH/2,game.WINDOWHEIGHT/2)
    def rotateit(self, ang):
        if not self.rotate:
            self.original = self.image
            self.rotate = True
    ...
    def move(self):
        ...
        if game.pressed_keys[K_UP] or game.pressed_keys[K_w]:
            self.backmove(self.x_speed, self.y_speed)
        ...
        if game.pressed_keys[K_RIGHT] or game.pressed_keys[K_d]:
            self.rotateit(2)
    def backmove(self,vx,vy):
        for i in game.allsp:
            i.rect.move_ip(-vx,-vy)
        for i in game.grounds:
            i.rect.move_ip(-vx, -vy)
    def move0(self):
        if game.pressed_keys[K_UP] or game.pressed_keys[K_w]:
            self.num = 0
        ...
        if self.rect.left <= 0:
            self.rect.left = 0
        elif self.rect.right >= game.WINDOWWIDTH:
            self.rect.right = game.WINDOWWIDTH
        if self.rect.top <= 0:
            self.rect.top = 0
        elif self.rect.bottom >= game.WINDOWHEIGHT:
            self.rect.bottom = game.WINDOWHEIGHT
    ...
    def ifgrow(self):
        ...
        if self.addscore >= 10:
            for i in range(self.addscore // 10):
                ...
                self.maxlive += 5
                self.shortltpor *=1.06
                self.firepower = self.firepower * 1.08
                self.ltpor = self.ltpor * 1.08
                self.bombpower = self.bombpower * 1.08
                self.birdpower += 0.1
                self.addlive(30)
    ...
    def hurt(self, power):
        ...
        Text(int(-power), (255, 0, 0), 60, self.rect.center)
...
class Damage(Text):
    live = 60
    name = 'arial'
    rgbs=[(255,255,0),(255,97,0),(65,105,225),(160,32,240),(255,0,0),(255,0,255),(25,25,112)]
    def __init__(self, owner, num):
        ...
        self.fontlist = [pygame.font.SysFont(self.name, i).render(str(num), True, rgb)
...
def load():
    ...
    Eexplosion.images = [pygame.transform.scale(load_image('electric.png'), (i, i)) for i in
                         [50, 90, 120, 150, 180, 210, 240, 270]]
    ...
    smallDirt.image = pygame.transform.smoothscale(load_image('defendimg.png'), (15, 15))
    # lightDirt.image=pygame.transform.smoothscale(load_image('lit.png'),(32,15))
    Light.images = [pygame.transform.smoothscale(load_image('lit.png'), (32, 15)),
                    pygame.transform.smoothscale(load_image('lit1.png'), (30, 15))]
...
class Game():
    ...
    energy = 40
    RUN = True
    SIZE = WINDOWWIDTH, WINDOWHEIGHT = 1200, 700
    running = 'r'
    pressed_keys = None
    pressed_mouse = None
    ...
    Player.containers =playerlist, tanks
...
while 1:
    ...
    screen = pygame.display.set_mode((game.WINDOWWIDTH, game.WINDOWHEIGHT), RESIZABLE)
    ...
    # Boss()
    if pygame.mixer:
        music = os.path.join(main_dir, "data", "music.mp3")
    ...
    while game.RUN:
        if game.running == 'r':
            ...
            game.WINDOWWIDTH, game.WINDOWHEIGHT = screen.get_size()
            ...
            for event in pygame.event.get():
                if event.type == KEYDOWN:
                    ...
                    if event.key == K_F11:
                        if not isfullscreen:
                            isfullscreen = True
                            SIZE = game.WINDOWWIDTH, game.WINDOWHEIGHT = pygame.display.list_modes()[0]
                            screen = pygame.display.set_mode(SIZE)
                            print(pygame.display.get_surface().get_size())
                        else:
                            isfullscreen = False
                            SIZE = game.WINDOWWIDTH, game.WINDOWHEIGHT = 1200, 700
                            screen = pygame.display.set_mode((game.WINDOWWIDTH, game.WINDOWHEIGHT), RESIZABLE)
