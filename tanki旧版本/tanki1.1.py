...
class Enemy(pygame.sprite.Sprite):
    ...
    def shoot(self):
        ...
        for i in range(4):
            self.bullet=BulletCoordinate(self.rect.centerx,self.rect.centery,i,True)
        ...
        game.enBulletgroup.add(self.bullet2)
        game.enBulletgroup.add(self.bullet3)
        game.enBulletgroup.add(self.bullet4)
        '''
    def update(self):
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if game.pressed_keys[K_SPACE]:
            self.shoot()
        if game.pressed_keys[K_f] and (game.energy>=10) and self.goldbody==False:
            self.golden()
        #if (game.pressed_keys[K_LSHIFT] or game.pressed_keys[K_RSHIFT]) and (game.energy>=20):            
            #self.killall()
        ...
        #if game.pressed_keys[K_k] and misslenum>0:
    def ifgrow(self):
    ...
    def golden(self):
        ...
        self.goldbody=True
        self.goldtime+=300
        #self.elecircle=Elecircle(self)
        #game.allsp.add(self.elecircle)
    ...
    def killall(self):
        ...
        for en2 in game.enemies:           
            en2.distory(1,0,1,0)
        for eb2 in game.enBulletgroup:
            eb2.kill()
    def distory(self):
...
class Elecircle(pygame.sprite.Sprite):
    def __init__(self,owner):
        super(Elecircle,self).__init__()
        self.owner=owner
        self.power=999
        self.rect = self.image.get_rect(center=self.owner.center)
    def update(self):
        self.rect = self.image.get_rect(center=self.owner.center)
